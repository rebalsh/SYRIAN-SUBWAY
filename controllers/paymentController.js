...
// ✅ **1. إنشاء جلسة دفع Stripe (محدث لدعم جميع أنواع التذاكر)**
exports.createStripePaymentSession = async (req, res) => {
  const transaction = await sequelize.transaction();
  ...
  try {
    ...
    const user_id = req.user.id;
    const { ticket_id, periodic_ticket_number, regular_ticket_group } = req.body;

    // 🔴 **التحقق: إما ticket_id أو periodic_ticket_number أو regular_ticket_group**
    if (!ticket_id && !periodic_ticket_number && !regular_ticket_group) {
      await transaction.rollback();
      return res.status(400).json({ 
        message: "مطلوب: إما ticket_id لتذكرة عادية أو periodic_ticket_number لتذكرة دورية أو regular_ticket_group لتذكرة عادية" 
      });
    }
    ...
    // 🔴 **الحالة 2: تذكرة عادية (regular_ticket_group)**
    else if (regular_ticket_group) {
      console.log(`🔍 البحث عن تذاكر عادية (group): ${regular_ticket_group}`);
      ...
      if (tickets.length === 0) {
        await transaction.rollback();
        return res.status(404).json({ 
          message: "لم يتم العثور على تذاكر عادية قيد الانتظار",
          regular_ticket_group 
        });
      }

      ticket = tickets[0];
      isRegular = true;
      console.log(`✅ تم العثور على ${tickets.length} تذكرة عادية`);
    }
    // 🔴 **الحالة 3: تذكرة عادية فردية (ticket_id)**
    else {
      console.log(`🔍 البحث عن تذكرة عادية: ${ticket_id}`);
    ...
    // 🔴 **بناء وصف الدفع حسب نوع التذكرة**
    let description = "";
    let ticketType = "";
    
    if (isPeriodic) {
      description = `تذكرة ${ticket.discount_type === 'weekly' ? 'أسبوعية' : 'شهرية'} - ${ticket.from_station.name} → ${ticket.to_station.name}`;
      ticketType = "periodic";
    } else if (isRegular) {
      description = `تذكرة مترو عادية (مجموعة) - ${ticket.from_station.name} → ${ticket.to_station.name}`;
      ticketType = "regular_group";
    } else {
      description = `تذكرة مترو عادية - ${ticket.from_station.name} → ${ticket.to_station.name}`;
      ticketType = "regular_single";
    }

    console.log(`💰 المبلغ: ${ticket.total_price} | النوع: ${ticketType}`);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: isPeriodic 
                ? `Metro Periodic Ticket (${ticket.discount_type})`
                : isRegular ? "Metro Regular Ticket Group" : "Metro Single Ticket",
              description: description,
              images: ["https://cdn-icons-png.flaticon.com/512/3097/3097140.png"],
            },
            unit_amount: Math.round(ticket.total_price * 100),
      ...
      metadata: {
        ...
        periodic_ticket_number: periodic_ticket_number || "none",
        regular_ticket_group: regular_ticket_group || "none",
        project: "metro_graduation_project"
      },
    ...
    if (isPeriodic) {
      ...
      // للتذاكر العادية (سواء مجموعة أو فردية)
      await Payment.create(
        {
          amount: ticket.total_price * tickets.length,
          status: "pending",
      ...
      console.log(`💰 تم تسجيل دفعة عادية بقيمة: ${ticket.total_price * tickets.length}`);
    }
...
async function processCompletedPayment(session, transaction) {
  try {
    ...
    // 🔴 **فحص نوع التذكرة ومعالجتها**
    if (ticket.is_periodic_ticket && ticket.periodic_ticket_number) {
      // تذكرة دورية
      await updatePeriodicTickets(ticket.periodic_ticket_number, session.payment_intent, transaction);
    } else if (ticket.periodic_ticket_number && !ticket.is_periodic_ticket) {
      // تذكرة عادية (group)
      await updateRegularTicketGroup(ticket.periodic_ticket_number, session.payment_intent, transaction);
    } else {
      // تذكرة عادية فردية
      await updateSingleTicket(ticket, session.payment_intent, transaction);
    }
...
// ✅ **5.1 تحديث مجموعة التذاكر العادية**
async function updateRegularTicketGroup(regularGroup, paymentReference, transaction) {
  const allTickets = await Ticket.findAll({
    where: {
      periodic_ticket_number: regularGroup,
      is_periodic_ticket: false,
      status: 'pending'
    },
    transaction,
  ...
  console.log(`🔄 تحديث ${allTickets.length} تذكرة عادية (group)`);
  ...
  console.log(`✅ تم تأكيد ${allTickets.length} تذكرة عادية (group)`);
}

// ✅ **6. تحديث تذكرة عادية فردية**
async function updateSingleTicket(ticket, paymentReference, transaction) {
  const qrData = JSON.stringify({
    ticket_id: ticket.id,
    ticket_number: ticket.ticket_number,
    ...
    from_station_id: ticket.from_station_id,
    to_station_id: ticket.to_station_id,
    trip_date: ticket.purchase_date,
...
async function handleTestWebhook(req, res) {
  ...
  try {
    ...
    const { session_id, periodic_ticket_number, regular_ticket_group } = req.body;
    
    if (!session_id && !periodic_ticket_number && !regular_ticket_group) {
      await transaction.rollback();
      return res.status(400).json({ 
        message: "أرسل إما session_id أو periodic_ticket_number أو regular_ticket_group" 
      });
    }
    ...
    } else if (periodic_ticket_number) {
      const ticket = await Ticket.findOne({
        where: { periodic_ticket_number, status: 'pending', is_periodic_ticket: true },
        transaction
      });
    ...
    if (ticket.is_periodic_ticket && ticket.periodic_ticket_number) {
      ...
      await updateRegularTicketGroup(ticket.periodic_ticket_number, "test_webhook", transaction);
    } else {
      await updateSingleTicket(ticket, "test_webhook", transaction);
    }
...
// ✅ **9. الحصول على تفاصيل الدفع (محدث)**
exports.getPaymentDetails = async (req, res) => {
  try {
    const { session_id, periodic_ticket_number, regular_ticket_group } = req.query;

    if (!session_id && !periodic_ticket_number && !regular_ticket_group) {
      return res.status(400).json({ message: "معرف الجلسة أو رقم التذكرة الدورية أو التذكرة العادية مطلوب" });
    }
    ...
    if (session_id) {
      ...
      // البحث برقم التذكرة الدورية
      const ticket = await Ticket.findOne({
        where: { periodic_ticket_number, is_periodic_ticket: true },
        include: [
          { model: Trip, include: [{ model: Line }] },
          { model: Station, as: "from_station" },
          { model: Station, as: "to_station" },
      ...
      if (!ticket) {
        return res.status(404).json({ message: "التذكرة الدورية غير موجودة" });
      }
      ...
      if (!ticket) {
        return res.status(404).json({ message: "التذكرة العادية غير موجودة" });
      }
...
// ✅ **10. اختبار دفع وهمي (للتطوير - محدث)**
exports.createMockPayment = async (req, res) => {
  const transaction = await sequelize.transaction();
  ...
  try {
    ...
    console.log(`🧪 بدء دفع وهمي - ticket_id: ${ticket_id}, periodic: ${periodic_ticket_number}, regular: ${regular_ticket_group}`);
    ...
    if (periodic_ticket_number) {
      ...
      if (tickets.length === 0) {
        await transaction.rollback();
        return res.status(404).json({ 
          message: "لم يتم العثور على تذاكر عادية",
          regular_ticket_group 
        });
      }
      ...
      ticket = tickets[0];
      isRegular = true;
      console.log(`✅ وجد ${tickets.length} تذكرة عادية`);
    } else {
      if (!ticket_id) {
        await transaction.rollback();
        return res.status(400).json({ message: "يرجى إرسال ticket_id أو periodic_ticket_number أو regular_ticket_group" });
      }
    ...
    if (isPeriodic) {
      ...
      console.log(`✅ تم تأكيد ${tickets.length} تذكرة عادية`);
    } else {
      const qrData = JSON.stringify({
        ticket_id: ticket.id,
        ticket_number: ticket.ticket_number,
      ...
      console.log(`✅ تم تأكيد تذكرة عادية فردية`);
    }
    ...
    res.json({
      ...
      note: "هذه وظيفة للتطوير فقط، لا تستخدم في الإنتاج",
      next_steps: [
        isPeriodic ? "استخدم /api/periodic-tickets/my-periodic-tickets لرؤية التذاكر" : 
        "استخدم /api/regular-tickets/my-tickets لرؤية التذاكر",
        "استخدم /api/payments/details للتحقق من حالة الدفع"
      ]
    });
...
// ✅ **11. تأكيد الدفع يدوياً (للتطوير - بديل Webhook - محدث)**
exports.manualConfirmPayment = async (req, res) => {
  const transaction = await sequelize.transaction();
  
  try {
    const user_id = req.user.id;
    const { periodic_ticket_number, regular_ticket_group, ticket_id } = req.body;

    console.log(`🔄 بدء تأكيد يدوي: ${periodic_ticket_number || regular_ticket_group || ticket_id}`);

    if (!periodic_ticket_number && !regular_ticket_group && !ticket_id) {
      await transaction.rollback();
      return res.status(400).json({ 
        message: "يرجى إرسال periodic_ticket_number أو regular_ticket_group أو ticket_id" 
      });
    }
    ...
    console.log(`🔍 وجد ${tickets.length} تذكرة للتأكيد (نوع: ${ticketType})`);

    // 🔥 تحديث التذاكر وإنشاء QR Codes
    for (const ticket of tickets) {
      let qrData;
      
      if (ticketType === "periodic") {
    ...
    const responseData = {
      ...
      next_steps: [
        ticketType === "periodic" 
          ? "استخدم /api/periodic-tickets/my-periodic-tickets لرؤية التذاكر المؤكدة"
          : "استخدم /api/regular-tickets/my-tickets لرؤية التذاكر المؤكدة",
        "استخدم /api/payments/details للتحقق من الدفع"
      ]
    };

    // إضافة معلومات إضافية حسب نوع التذكرة
    if (ticketType === "periodic") {
      responseData.summary.periodic_ticket_number = periodic_ticket_number;
      responseData.summary.period_type = firstTicket.discount_type === 'weekly' ? 'أسبوعي' : 'شهري';
      responseData.summary.valid_from = firstTicket.periodic_start_date;
      responseData.summary.valid_until = firstTicket.periodic_end_date;
    } else if (ticketType === "regular") {
      responseData.summary.regular_ticket_group = regular_ticket_group;
      responseData.summary.ticket_type = "عادية (مجموعة)";
    } else {
      responseData.summary.ticket_type = "عادية (فردية)";
      responseData.summary.ticket_id = ticket_id;
    }
    ...
    res.status(500).json({
      message: "خطأ في التأكيد اليدوي",
      error: err.message,
      suggestion: "تأكد من صحة رقم التذكرة"
    });
  }
};
...
exports.refundTicket = async (req, res) => {
  ...
  try {
    ...
    if (!ticket_id && !periodic_ticket_number && !regular_ticket_group) {
      await transaction.rollback();
      return res.status(400).json({ 
        message: "أرسل إما ticket_id أو periodic_ticket_number أو regular_ticket_group" 
      });
    }
