...
exports.getAllLines = async (req, res) => {
  try {
    ...
    // 🔴 أضفنا هذا السطر فقط: تحديد الحد الأقصى للنتائج
...
exports.getLinePrices = async (req, res) => {
  try {
    ...
    const lines = await Line.findAll({
      attributes: ["id", "line_name", "price"],
      order: [["line_name", "ASC"]],
      limit: 100 // 🔴 أضفنا limit هنا أيضاً
    });
...
// 🔴 دالة جديدة للاختبار فقط (لا تؤثر على التطبيق)
exports.getLinesPerformance = async (req, res) => {
  try {
    const lines = await Line.findAll({
      attributes: ["id", "line_name", "price"],
      limit: 50,
    ...
    res.json({
      success: true,
      message: "اختبار الأداء - لا يؤثر على التطبيق",
      data: lines,
      count: lines.length
    });
  } catch (err) {
